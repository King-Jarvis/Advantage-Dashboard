...
| Prompt injection via email or payee text | Untrusted content delimited and named as untrusted; output parsed as strict JSON; a category name that is not already yours is discarded; importance clamped 1–5; output never drives control flow |
...
- **Prompt injection cannot be eliminated**, only bounded. Constraining the output shape
  and keeping it out of control flow limits the worst case to a wrong importance score or
  a merchant filed under a category you already created.
