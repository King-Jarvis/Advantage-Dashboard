...
def test_the_side_a_merchant_sits_on_follows_its_category(live):
    """Move a positive-amount merchant to an expense category and it is an
    expense now, whatever the refund that started it looked like. Banding by
    the amount sign would contradict the category just chosen."""
    from dashboard import ledger
    from dashboard import storage as st
    conn = st.connect()
    gexp = ledger.create_category_group(conn, "Everyday")
    ginc = ledger.create_category_group(conn, "Income", is_income=True)
    subs = ledger.create_category(conn, gexp, "Subscriptions")
    xfer = ledger.create_category(conn, ginc, "Transfer In", is_income=True)
    acct = ledger.create_account(conn, "Checking")
    # Positive amount: a refund, so the sign says "money in".
    ledger.add_transaction(conn, acct, "2026-08-01", 3246, "AMAZON PRIME 4471")
    conn.close()
    ...
    call(live, "POST", "/api/edit/by-payee",
         {"payee_key": "amazon prime", "category_id": xfer}, headers=h)
    _, _, got = call(live, "GET", "/api/view/unfiled?filed=1",
                     headers={"Cookie": cookie})
    g = next(x for x in got["groups"] if x["key"] == "amazon prime")
    assert g["is_income"] is True, "an income category should read as money in"

    call(live, "POST", "/api/edit/by-payee",
         {"payee_key": "amazon prime", "category_id": subs, "overwrite": True},
         headers=h)
    _, _, got = call(live, "GET", "/api/view/unfiled?filed=1",
                     headers={"Cookie": cookie})
    g = next(x for x in got["groups"] if x["key"] == "amazon prime")
    assert g["is_income"] is False, "still reading as income after the move"
    assert g["category"] == "Subscriptions"
    assert g["total_cents"] > 0, "the amount itself has not changed"


def test_an_unfiled_merchant_falls_back_to_the_amount_sign(live):
    """Before a category exists there is nothing else to go on."""
    from dashboard import ledger
    from dashboard import storage as st
    conn = st.connect()
    acct = ledger.create_account(conn, "Checking")
    ledger.add_transaction(conn, acct, "2026-08-01", 5000, "SOME REFUND 4471")
    conn.close()
    cookie, _ = login(live)
    _, _, got = call(live, "GET", "/api/view/unfiled", headers={"Cookie": cookie})
    g = next(x for x in got["groups"] if x["key"] == "some refund")
    assert g["is_income"] is None and g["total_cents"] > 0
